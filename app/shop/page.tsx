  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="border-b border-brand-pink/40 pb-8 text-center sm:text-left">
        ...
        <p className="mt-2 text-xs sm:text-sm text-brand-dark/60">
      ...
      <div className="mt-8 flex flex-col md:flex-row gap-4 justify-between items-center bg-white border border-brand-pink/30 rounded-2xl p-4 shadow-xs">
        ...
        <div className="relative w-full md:max-w-md">
          ...
          <input
            ...
            className="w-full bg-brand-cream/40 border border-brand-pink/50 rounded-full pl-10 pr-4 py-2.5 text-xs text-brand-dark placeholder-brand-dark/40 focus:outline-none focus:border-brand-terracotta"
          />
        </div>

        {/* Sorting Dropdown selector */}
        <div className="flex gap-3 w-full md:w-auto items-center justify-end">
          <span className="text-xs font-semibold text-brand-dark/60 uppercase tracking-wider">Sort By:</span>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value)}
            className="bg-brand-cream/40 border border-brand-pink/50 rounded-full px-4 py-2 text-xs text-brand-dark font-medium focus:outline-none focus:border-brand-terracotta cursor-pointer"
          >
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <aside className="col-span-1 bg-white border border-brand-pink/30 rounded-2xl p-6 shadow-xs flex flex-col gap-6">
          ...
          {/* Category Filters */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-brand-dark/80 mb-3">Category</h4>
            <div className="flex flex-col gap-2">
              <button
                onClick={() => setSelectedCategory('')}
                className={`text-left text-xs py-1.5 px-3 rounded-lg font-medium transition-all ${
                  selectedCategory === '' 
                    ? 'bg-brand-pink text-brand-terracotta' 
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-xs font-bold uppercase tracking-widest text-brand-dark/80">Max Price</h4>
            ...
            />
            <div className="flex justify-between text-[10px] text-brand-dark/40 font-semibold mt-1">
              <span>$20</span>
              <span>$100</span>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-brand-dark/80 mb-3">Fabric / Print</h4>
              ...
              <button
                ...
                className={`text-xs px-2.5 py-1.5 rounded-full border transition-all ${
                  selectedFabric === ''
                    ...
                    : 'bg-white border-brand-pink/60 text-brand-dark/70 hover:border-brand-terracotta/40'
                }`}
              >
                All Prints
              </button>
        ...
        <main className="col-span-1 lg:col-span-3">
          {loading ? (
            ...
            <div className="bg-white border border-brand-pink/30 rounded-2xl py-16 px-4 text-center flex flex-col items-center shadow-xs">
              <p className="font-serif text-lg text-brand-dark/80">No products match your current filters.</p>
              <p className="text-xs text-brand-dark/50 mt-1 mb-6">Try resetting filters or checking spelling.</p>
              <button
                onClick={handleResetFilters}
